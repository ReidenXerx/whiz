...
- **NS-14** — Poisoned calibration: speech filling the 1 s window drives the gate to ≈3× speech RMS and drops the first word in BOTH implementations; the corpus pins this as current expected behavior, so a fix MUST update the corpus in the same commit. Directions (skip speech-energy frames in calibration; per-frame outlier rejection) sketched, undecided. — src: docs/ARCHITECTURE.md, tuning/golden `speech_during_calibration`
